...
infer_latent_factors = function(Y, X, K, alpha = 5, max_iter = 100, burnin = max_iter/2, keep_Y_trace = TRUE, which_genes = NULL, which_samples = NULL){
  ...
  for(i in 1:max_iter){
    # update Z
    ZZ = update_Z(Y, Z, ZZ, W, gamma, intercept_Y, lambda, G, K, N, alpha)
    Z = 1*(ZZ != 0)
    ...
    # update W
    sigma_W_inv = precision_W * diag(K) + fast_outer_product_sum(gamma, Z, Z, K, K, G)
    sigma_W = solve(sigma_W_inv)
    temp_W = precision_W * outer(beta0, rep(1, N)) + precision_W * outer(beta, colSums(X)) + fast_outer_product_sum(gamma, Z, Y, K, N, G)
    mu_W = sigma_W %*% temp_W
    ...
    # Ypred without noise
    Ypred0 = ZZ %*% W + intercept_Y
    
    # update beta
    x_colsums = colSums(X)
    sigma_beta = 1 / (rho + sum(precision_W * x_colsums**2))
    mu_beta = sigma_beta * colSums(precision_W * x_colsums * t(W))
    beta = mu_beta + rnorm(K, 0, sqrt(sigma_beta))
    # beta0
    sigma_beta = 1 / (rho + precision_W * N)
    mu_beta = sigma_beta * rowSums(precision_W * (W - outer(beta, x_colsums)))
    beta0 = mu_beta + rnorm(K, 0, sqrt(sigma_beta))
    
    # update precision parameters
    a_precision_W = a0 + 0.5*K*N
    b_precision_W = b0 + 0.5*sum((W - beta0 - outer(beta, x_colsums))**2)
    precision_W = rgamma(1, a_precision_W, b_precision_W)
    a_gamma = a0 + 0.5*N
    ...
    if(i > burnin){
      ...
      intercept_mean = intercept_mean + 1/(max_iter-burnin) * intercept_Y
      ...
      # cat(sprintf("range gamma (%1.3f, %1.3f), mean(gamma): %1.3f, precision_W: %1.3f\n", min(gamma), max(gamma), mean(gamma), precision_W))
      
      # loglikelihood
      loglik_Z = calculate_IBP_prob(alpha, m_k, K, G, log = TRUE)
      loglik_W = sum(dnorm(W, beta0 + outer(beta, x_colsums), 1/sqrt(precision_W), log=TRUE))
